...
//
// This one file drives five boards, two of which (X4, X3) share one
// ESP32-C3 binary (env:xteink) picked at runtime by freeink::
// selectXteinkDevice() in setup(); the rest (X4 Pro, Murphy M3, M5Stack
// PaperS3) are each their own ESP32-S3 binary (env:xteink_x4pro, env:murphy,
// env:papers3) but run the same source. Where a board has real hardware the
...
// see docs/board-notes/murphy-m3.md and docs/board-notes/papers3.md for
// those two boards' port-specific findings. PaperS3 has NO physical buttons
// at all — see the "PaperS3 touch-only navigation" block near handleInput()
// below.
//
...
static void drawInfoPage(uint32_t now, int16_t y) {
  ...
  if (infoPage == 0) {
    ...
    // otherwise need this line touched again (it already had to be, twice).
